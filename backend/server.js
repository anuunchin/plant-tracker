...
app.post('/plants', (req, res) => {                 //handles POST requests to the /plants endpoint
    ...
    if (!wateringInterval || isNaN(wateringInterval) || wateringInterval <= 0) {
        return res.status(400).send({ error: 'Invalid or missing watering interval' });
    }

    const plant = { name, lastWatered, id: uuidv4(), wateringHistory: [lastWatered], wateringInterval}; 
